...
module.exports = function(treatmentName, settings, stager, setup, gameRoom) {
    ...
    stager.setOnInit(function() {
        ...
        this.pairnumber = 10;//the number of pairs in the total experiment, should be 57
    ...
    //                 this.finalGuesses = node.widgets.append('CustomInputGroup', W.gid('containerbottom'), {//create customInputGroup widget for clue options, only the first is mandatory
    //                    id: 'finalGuesses',
    //                    orientation: 'H',
    //                    required: true,
    ...
    //                        {
    //                            id: 'clue1',
    //                            mainText: 'Please type your FINAL clue below and click Done:',
    //                            requiredChoice: true
    //                        },
    //                     //    {
    //                     //        id: 'clue2',
    ...
    //                 // this.cluespast.push(this.clueGive.getValues().value);

    //                 // node.say('CLUE', node.game.partner, this.clueGive2.getValues().value);

    //                 // node.set({clueFinal : this.clueGive.getValues().value});
    //                 // node.set({TBFinal : this.clueGive.getValues().timeBegin});
    //                 // node.set({TEFinal : this.clueGive.getValues().timeEnd});

    //                 // return;


    //                 // node.set({clueOption1 : this.finalGuesses.getValues().items['clue1'].value}),
    //                 // node.set({TBOption1 : this.finalGuesses.getValues().items['clue1'].timeBegin}),
    //                 // node.set({TEOption1 : this.finalGuesses.getValues().items['clue1'].timeEnd}),

    //                 // node.set({clueOption2 : this.finalGuesses.getValues().items['clue2'].value}),
    //                 // node.set({TBOption2 : this.finalGuesses.getValues().items['clue2'].timeBegin}),
    //                 // node.set({TEOption2 : this.finalGuesses.getValues().items['clue2'].timeEnd}),

    //                 // node.set({clueOption3 : this.finalGuesses.getValues().items['clue3'].value}),
    //                 // node.set({TBOption3 : this.finalGuesses.getValues().items['clue3'].timeBegin}),
    //                 // node.set({TEOption3 : this.finalGuesses.getValues().items['clue3'].timeEnd}),

    //                 // node.set({clueOption4 : this.finalGuesses.getValues().items['clue4'].value}),
    //                 // node.set({TBOption4 : this.finalGuesses.getValues().items['clue4'].timeBegin}),
    //                 // node.set({TEOption4 : this.finalGuesses.getValues().items['clue4'].timeEnd}),

    //                 // node.set({clueOption5 : this.finalGuesses.getValues().items['clue5'].value}),
    //                 // node.set({TBOption5 : this.finalGuesses.getValues().items['clue5'].timeBegin}),
    //                 // node.set({TEOption5 : this.finalGuesses.getValues().items['clue5'].timeEnd}),

    //                 // node.set({clueOption6 : this.finalGuesses.getValues().items['clue6'].value}),
    //                 // node.set({TBOption6 : this.finalGuesses.getValues().items['clue6'].timeBegin}),
    //                 // node.set({TEOption6 : this.finalGuesses.getValues().items['clue6'].timeEnd}),

    //                 // node.set({clueOption7 : this.finalGuesses.getValues().items['clue7'].value}),
    //                 // node.set({TBOption7 : this.finalGuesses.getValues().items['clue7'].timeBegin}),
    //                 // node.set({TEOption7 : this.finalGuesses.getValues().items['clue7'].timeEnd}),

    //                 // node.set({clueOption8 : this.finalGuesses.getValues().items['clue8'].value}),
    //                 // node.set({TBOption8 : this.finalGuesses.getValues().items['clue8'].timeBegin}),
    //                 // node.set({TEOption8 : this.finalGuesses.getValues().items['clue8'].timeEnd}),

    //                 // node.say('CLUES', node.game.partner, this.finalGuesses.getValues().items['clue1'].value);//this sends a signal to the other player so their step ends
    //                 this.cluespast.push(this.clueGive.getValues().value);
    ...
    //                 return;
    //                 node.say('CLUES', node.game.partner);//this sends a signal to the other player so their step ends
    //                 return;
    //             }
        ...
        roles: {
            CLUEGIVER:{
                ...
                cb: function() {
                    ...
                    node.on.data('GUESS1', function(msg) {
                        that.guess1Received = msg.data.toUpperCase();
                    });
                    node.on.data('GUESS2', function(msg) {
                        that.guess2Received = msg.data.toUpperCase();
                        node.done();
                    });
                }
            },
            GUESSER:{
                ...
                //   }
                  ...
                  this.finalGuesses = node.widgets.append(
                    "CustomInputGroup",
                    W.gid("containerbottomguesses"),
                    {
                      //create customInputGroup widget for clue options, only the first is mandatory
                      id: "finalGuesses",
                      orientation: "H",
                      required: true,
                      ...
                      items: [
                        {
                          id: "finalGuess1",
                          mainText: "Please type in your guess: Word 1 ",
                          requiredChoice: true,
                        },
                        {
                          id: "finalGuess2",
                          mainText: "Please type in your guess: Word 2",
                          requiredChoice: true,
                        },
                        //    {
                        //        id: 'clue3',
                      ...
                      //If any validation is necessary for final guess
                      validation: function (res, values) {
                        // Custom validation (only reports about last word).

                        // if (values.finalGuess2.length>=0) {
                        //     res.err = 'Your guess one is' + values.finalGuess1;
                        // }

                        // if (values.finalGuess2.length>=0) {
                        //     res.err = 'Your guess two is' + values.finalGuess2;
                        // }
                        //    if (values.clue3 && node.game.inArrayCaseInsensitive(values.clue3, node.game.boardboard[node.game.roundCounter])) {
                        //        res.err = 'You have used a forbidden word: ' + values.clue3;
                        //    }
                      ...
                      onfocusout: function (res, input, that) {
                        // var values = {};
                        // values[input.it] = res;
                        // that.validation(res, values);
                        var myDiv = W.getElementById("alist");
                        ...
                        if (myDiv.innerHTML == " Your final answers:  ") {
                          ...
                          node.set({GUESS_1_FINAL_TIME: node.timer.getTimeSince("step")});
                          node.game.memory.add({
                            //adds the responded values to memory so we can access it later, cannot store this info in a variable since it is in an event listener
                            player: node.player.id,
                            stage: node.game.getCurrentGameStage(),
                            Guess1: finalGuess1,
                          });
                    ...
                    let finalGuess1 =  this.finalGuesses.getValues().items["finalGuess1"].value.toUpperCase();  
                    let finalGuess2 =  this.finalGuesses.getValues().items["finalGuess2"].value.toUpperCase();  
                    // node.set({guessOption1 : this.finalGuesses.getValues().items['finalGuess1'].value});
                    // node.set({TBGuessOption1 : this.finalGuesses.getValues().items['finalGuess1'].timeBegin});
                    // node.set({TEGuessOption1 : this.finalGuesses.getValues().items['finalGuess1'].timeEnd});
                    node.say('GUESS1', node.game.partner, finalGuess1);
                    node.set({GUESS_1_FINAL : finalGuess1});
                    node.set({GUESS_1_FINAL_TIME : node.timer.getTimeSince('step')})
                    node.game.memory.add({//adds the responded values to memory so we can access it later, cannot store this info in a variable since it is in an event listener
                    ...
                    W.gid("containerbottomguesses"),
                    {
                      ...
                      onfocusout: function (res, input, that) {
                        ...
                        if (myDiv.innerHTML == " Your final answers:  ") {
                          ...
                          myDiv.innerHTML =
                            myDiv.innerHTML +
                            this.finalGuesses.getValues().items["finalGuess1"]
                              .value;
                          node.say("GUESS1",node.game.partner,this.finalGuesses.getValues().items["finalGuess1"].value);
                          node.set({ GUESS_1_FINAL: this.finalGuesses.getValues().items["finalGuess1"].value });
                          node.set({GUESS_1_FINAL_TIME: node.timer.getTimeSince("step")});
                          node.game.memory.add({
                            //adds the responded values to memory so we can access it later, cannot store this info in a variable since it is in an event listener
                            player: node.player.id,
                            stage: node.game.getCurrentGameStage(),
                            Guess1: this.finalGuesses.getValues().items["finalGuess1"].value,
                          });
                          node.game.memory.tag("guess1");
                        }
                        else if(!myDiv.innerHTML.includes(",")) {//the condition if 1 word has been added, stores the second word, send it to the partner, and ends the step for both players
